  ...
  reader.onload = async (e) => {
    try {
      ...
      showConfigPreviewModal(settings, async () => {
        await saveImportedSettings(settings, importVersion, showStatus);
      });
...
async function downloadFromWebDAV() {
  ...
  try {
    ...
    if (result.success) {
      ...
      // 弹出预览确认框
      showConfigPreviewModal(settings, async () => {
        await saveImportedSettings(settings, remoteVersion, (msg, type) => {
          if (type === "success") {
            showWebDAVStatus(`✅ 配置已从 WebDAV 下载并导入（版本号：${remoteVersion}）`, "success");
          } else {
            showWebDAVStatus("❌ 导入失败: " + msg.replace("导入保存失败: ", ""), "error");
          }
        });
      });
